...
Providers: local sentence-transformers, OpenAI-compatible endpoints, Google Gemini, MiniMax, Voyage, and OrcaRouter.
Configure via provider/model parameters, CRG_EMBEDDING_MODEL for local, CRG_OPENAI_* for OpenAI-compatible endpoints, ORCAROUTER_API_KEY for OrcaRouter, or VOYAGE_API_KEY plus optional CRG_VOYAGE_MODEL for Voyage.
</section>
